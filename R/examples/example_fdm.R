...
# - non-equispaced mesh improve accuracy, use: alpha = 0.1
...
# k-grid
min.s <- 0 # S0*exp((r[nt]-q[nt]-0.5*0.2^2)*mat - 5*0.2*sqrt(mat)) 
max.s <- S0*exp((r[nt]-q[nt]-0.5*0.2^2)*mat + 4*0.2*sqrt(mat)) 

# construct space-grid: equispaced vs. hyperbolic 
k_grid <- seq(min.s, max.s, length.out=nk-1)
k_grid2 <- hyperbolicMesh(min.s, max.s, c=S0, n=(nk-1), alpha=0.1)

k_grid2 <- hyperbolicMesh(min.s, max.s, c=S0, n=nk, alpha=0.1, x_star=S0)
k_grid2$x[k_grid2$idx]-590
...
# declare 
back_cn_dicrete <- back_mix_dicrete <- back_imp_dicrete <- back_cn_cont <- back_mix_cont <- back_imp_cont <- matrix(NA,length(K))

# Backward FDM (continous time version)
for(i in 1:length(K)){
...
# plot 
idx <- cn$s_grid>300 & cn$s_grid<900
plot(cn$delta[idx], x=cn$s_grid[idx], type="l", lty=6, ylab = "Delta", xlab = "Spot")
lines(imp$delta[idx], x=imp$s_grid[idx], col="green", lty = 5)
lines(mix$delta[idx], x=mix$s_grid[idx], lty=2, col = "red")
